...
# find out how many arriving species were seen prior to 2011
# (to address reviewer criticism on methodological change in sampling)
arriving_spp <- as.character(pa_pivot2_summary$organism[pa_pivot2_summary$org_group == "Arriving"])
pa %>% 
  filter(organism %in% arriving_spp) %>%
  group_by(organism) %>%
...
pa_pivot2_summary %>% 
  count(perc_monthly_max_95 < maxtemp_start)
