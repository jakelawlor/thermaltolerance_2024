...
# NOTE this script grouped occurrences into 5-year bins,
# instead, we moved on from that and regressed raw yearly occurrences
# in the next script. 
...
# upload full PA (not highly sampled) becuase we will get
# the total spread of years from that, as the highly sampled 
# dataset exludes more years. 
pa <- readr::read_csv(
#  here::here(
#    "data-processed",
#    "appledore-survey-data",
...
all_p %>% select(-contains("sig"), -any_p) %>%
  tidyr::pivot_longer(cols = c(mean_p, mean_trend, 
                      ...
                      names_sep = "_") %>%
  mutate(sig = case_when(p < .05 & trend > 0 ~ "Shift Up",
                         p < .05 & trend < 0 ~ "Shift Down",
                         p > .05 ~ "No Change")) %>%
...
hs_depth_bars <- hs_all_p2 %>%
  ...
  geom_bar(color = "black", width = 1,
           linewidth = .2) +
  coord_flip(ylim = c(0,length(unique(hs_all_p2$organism))),
             xlim = c(.5, 4.5),
             expand = F) +
  scale_x_discrete(labels = ~paste(stringr::str_to_title(.),"\nObservation \nHeight")) +
  scale_fill_manual(breaks = c("Sig Shift Down",
  ...
  labs(x = NULL,
       ...
       fill = "Modeled Shift") +
  theme(legend.position = "bottom")  +
  geom_hline(yintercept = length(unique(hs_all_p2$organism))/2,
             linetype = "dashed") +
  guides(fill = guide_legend(theme= theme(legend.title.position = "top"))) +
  theme(legend.margin = margin(t = -15,0,0,0,"pt"))
...
hs_unc_sum_split <- hs_unc_sum %>% split(f = .$organism)

# model and assess mean observation heights
hs_unc_mean_mods <- purrr::map(
  .x = hs_unc_sum_split,
  .f = ~lm(data = .x, 
           formula = "heightmean ~ year")
)
hs_unc_mean_mods_p <- purrr::map(
  .x = hs_unc_mean_mods,
  .f = ~ pf(summary(.x)$fstatistic[1], summary(.x)$fstatistic[2], summary(.x)$fstatistic[3], lower.tail = FALSE)
) %>%
  bind_rows(.id = "organism") %>%
  rename(mean_p = value)
hs_unc_mean_mods_trend <- purrr::map(
  .x = hs_unc_mean_mods,
  .f = ~ data.frame(trend_value = summary(.x)$coefficients["year","Estimate"],
                    trend_sign = ifelse(sign(summary(.x)$coefficients["year","Estimate"]) == 1,
                                        "(+)",
                                        "(-)"
                    )) )%>%
  bind_rows(.id = "organism") %>%
...
# model and assess .05 quantile observation heights
hs_unc_min_mods <- purrr::map(
  .x = hs_unc_sum_split,
  .f = ~lm(data = .x, 
           formula = "height05 ~ year")
)
hs_unc_min_mods_p <- purrr::map(
  .x = hs_unc_min_mods,
  .f = ~ pf(summary(.x)$fstatistic[1], summary(.x)$fstatistic[2], summary(.x)$fstatistic[3], lower.tail = FALSE)
) %>%
  bind_rows(.id = "organism") %>%
  rename(min_p = value)
hs_unc_min_mods_trend <- purrr::map(
  .x = hs_unc_min_mods,
  .f = ~ data.frame(trend_value = summary(.x)$coefficients["year","Estimate"],
                    trend_sign = ifelse(sign(summary(.x)$coefficients["year","Estimate"]) == 1,
                                        "(+)",
  ...
  rename(min_trend = trend_value,
         min_sign = trend_sign)
hs_unc_min_perfect <- purrr::map(
  .x = hs_unc_sum_split,
  .f = ~ifelse(length(unique(.x$height05)) == 1,"single value","no issue")
)
hs_unc_min_perfect %>% unlist() %>% table()

# model and assess .95 quantile observation heights
hs_unc_max_mods <- purrr::map(
  .x = hs_unc_sum_split,
  .f = ~lm(data = .x, 
           formula = "height95 ~ year")
)
hs_unc_max_mods_p <- purrr::map(
  .x = hs_unc_max_mods,
  .f = ~ pf(summary(.x)$fstatistic[1], summary(.x)$fstatistic[2], summary(.x)$fstatistic[3], lower.tail = FALSE)
) %>%
  bind_rows(.id = "organism") %>%
  rename(max_p = value)
hs_unc_max_mods_trend <- purrr::map(
  .x = hs_unc_max_mods,
  .f = ~ data.frame(trend_value = summary(.x)$coefficients["year","Estimate"],
                    trend_sign = ifelse(sign(summary(.x)$coefficients["year","Estimate"]) == 1,
                                        "(+)",
  ...
  rename(max_trend = trend_value,
         max_sign = trend_sign)
hs_unc_max_perfect <- purrr::map(
  .x = hs_unc_sum_split,
  .f = ~ifelse(length(unique(.x$height95)) == 1,"single value","no issue")
)
hs_unc_max_perfect %>% unlist() %>% table()


# model and assess median  observation heights
hs_unc_med_mods <- purrr::map(
  .x = hs_unc_sum_split,
  .f = ~lm(data = .x, 
           formula = "heightmed ~ year")
)
hs_unc_med_mods_p <- purrr::map(
  .x = hs_unc_med_mods,
  .f = ~ pf(summary(.x)$fstatistic[1], summary(.x)$fstatistic[2], summary(.x)$fstatistic[3], lower.tail = FALSE)
) %>%
  bind_rows(.id = "organism") %>%
  rename(med_p = value)
hs_unc_med_mods_trend <- purrr::map(
  .x = hs_unc_med_mods,
  .f = ~ data.frame(trend_value = summary(.x)$coefficients["year","Estimate"],
                    trend_sign = ifelse(sign(summary(.x)$coefficients["year","Estimate"]) == 1,
                                        "(+)",
  ...
  rename(med_trend = trend_value,
         med_sign = trend_sign)
hs_unc_med_perfect <- purrr::map(
  .x = hs_unc_sum_split,
  .f = ~ifelse(length(unique(.x$heightmed)) == 1,"single value","no issue")
)
hs_unc_med_perfect %>% unlist() %>% table()

# merge all p values ------------------------------------------------------
hs_unc_all_p <- hs_unc_mean_mods_p %>%
  left_join(hs_unc_mean_mods_trend) %>%
  left_join(hs_unc_max_mods_p) %>%
  left_join(hs_unc_max_mods_trend) %>%
  left_join(hs_unc_min_mods_p) %>%
  left_join(hs_unc_min_mods_trend) %>%
  left_join(hs_unc_med_mods_p) %>%
  left_join(hs_unc_med_mods_trend)


hs_unc_all_p <- hs_unc_all_p %>%
  mutate(mean_p_sig = case_when(mean_p < .05 ~ paste0("<b>mean ",mean_sign,"</b><br>"),
                                TRUE ~ ""),
         max_p_sig = case_when(max_p < .05 ~ paste0("<b>max ",max_sign,"</b><br>"),
                               TRUE ~ ""),
