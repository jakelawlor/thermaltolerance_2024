...
formulas_list.max <- list(
  # first, test relationships with no accounting for dataset
  cti.max ~ year + tidalheight,
  cti.max ~ year * tidalheight,
  cti.max ~ year + I(tidalheight^2),
  cti.max ~ year * I(tidalheight^2),
  # then, with dataset added as a categorical factor
  cti.max ~ (year + tidalheight) + df,
  cti.max ~ (year * tidalheight) + df,
  cti.max ~ (year + I(tidalheight^2)) + df,
  cti.max ~ (year * I(tidalheight^2)) + df,
  # then, with dataset as an interacting categorical factor
  cti.max ~ (year + tidalheight) * df,
  cti.max ~ (year * tidalheight) * df,
  ...
  cti.max ~ year + (tidalheight*df),
  cti.max ~ year + (I(tidalheight^2)*df),
  cti.max ~ (year*df) + tidalheight,
  cti.max ~ (year*df) + I(tidalheight^2)
...
performance::compare_performance(mods.max, rank = T)


formulas_list.max$mod5
mergemod.max <- mods.max$mod5
summary(mergemod.max)
mergemod.max



au_merge.max <- broom::augment(mergemod.max,
                           newdata = pred_df)
...
formulas_list.min<- list(
  # first, test relationships with no accounting for dataset
  cti.min~ year + tidalheight,
  cti.min~ year * tidalheight,
  cti.min~ year + I(tidalheight^2),
  cti.min~ year * I(tidalheight^2),
  # then, with dataset added as a categorical factor
  cti.min~ (year + tidalheight) + df,
  cti.min~ (year * tidalheight) + df,
  cti.min~ (year + I(tidalheight^2)) + df,
  cti.min~ (year * I(tidalheight^2)) + df,
  # then, with dataset as an interacting categorical factor
  cti.min~ (year + tidalheight) * df,
  cti.min~ (year * tidalheight) * df,
  ...
  cti.min~ year + (tidalheight*df),
  cti.min~ year + (I(tidalheight^2)*df),
  cti.min~ (year*df) + tidalheight,
  cti.min~ (year*df) + I(tidalheight^2)
...
performance::compare_performance(mods.min, rank = T)


formulas_list.min$mod10
mergemod.min<- mods.min$mod10
summary(mergemod.min)
mergemod.min



au_merge.min<- broom::augment(mergemod.min,
                               newdata = pred_df)
...
(p.max | p | p.min) + ggview::canvas(12,4)
...
# summarize points to one
df_merge2 <- df_merge %>%
  group_by(year, tidalheight) %>%
  summarize(cti = weighted.mean(cti, n_spp),
            n_spp = sum(n_spp),
            n_df = n_distinct(df)) 
  filter(n_df == 2)
...
merge2mod1 <- lm(data = df_merge2,
                 formula = cti ~ year + tidalheight)
summary(merge2mod1)

merge2mod2 <- lm(data = df_merge2,
                 formula = cti ~ year * tidalheight)
summary(merge2mod2)

merge2mod3 <- lm(data = df_merge2,
                 formula = cti ~ year + I(tidalheight^2))
summary(merge2mod3)

merge2mod4 <- lm(data = df_merge2,
                 formula = cti ~ year * I(tidalheight^2))
summary(merge2mod4)

merge2mod5 <- lm(data = df_merge2,
                 formula = cti ~ year )
summary(merge2mod5)

performance::compare_performance(merge2mod1,
                                 merge2mod2,
                                 merge2mod3, 
                                 merge2mod4,
                                 merge2mod5, 
...
au_merge2 <- broom::augment(merge2mod4,
                           newdata = pred_df %>% filter(df == "cover"))
au_merge2 %>% 
  #left_join(df_merge) %>%
  arrange(year) %>%
  ggplot(aes(x = .fitted,
             y = tidalheight, group = tidalheight)) +
  geom_point(data = df_merge2,
             aes(x = cti,
                 color = year),
             shape= "|",
             size= 2.5,
