...
temps <- readr::read_csv("data-processed/appledore_temps_1982_2023.csv") %>%
...
filtered_transect_p <- pa_therm %>% distinct(transect_label, tidalheight, year) %>%
  ...
  labs(y = "Tidal Level (m)",
       x = NULL,
       title = "Retained Highly Sampled Transects for CTI") +
...
ggsave(filtered_transect_p,
       filename = "outputs/cti/highly_sampled_transects.png",
       width = 8,
       height = 3.5)
...
readr::write_csv(pa_therm,
                 file = "data-processed/cti-data/cti-data-all.csv")
readr::write_csv(pa_therm_filtered,
                 file = "data-processed/cti-data/cti-data-highly-sampled.csv")
saveRDS(tempmod,
        file = "data-processed/cti-data/temp-change-model.rds")
readr::write_csv(temptrend,
        file = "data-processed/cti-data/temp-change-augment.csv")
rm(list = ls())
