...
rm(unique, y1,y2,y3,y4,y5,y6,y7,y8)
...
pa_hs <- readr::read_csv("data-processed/cti-data/cti-data-highly-sampled.csv")%>%
  tidyr::separate(organism, into = c("Genus","Species")) %>%
  mutate(G = paste0(stringr::str_sub(Genus,1,1),".")) %>%
  mutate(organism = paste(G, Species)) %>%
  select(-G, -Genus, -Species)
...
ggplot() +
  geom_richtext(aes(x = 0, y = 0, label = "<span style='color:blue'>p < .05</span><span style='color:red'>p < .05</span>"))
